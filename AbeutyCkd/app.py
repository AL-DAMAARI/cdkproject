...
#loading model
model = pickle.load(open('model_rf.pkl','rb')) # data ku train gresay ayuu kuso qada
scaler = pickle.load(open('scaler.pkl','rb'))  # jaam ku yrena
...
# mesha ay iskaga xeran yahin machine learning ka io front end
def prediction(sg, rbc, pc, sod, hemo, pcv, rc, htn, scaler, model):
    ...
    final_features = [np.array(int_features)]  # xogta kama dnbeska oo model ka smen laha aya ku shubay
